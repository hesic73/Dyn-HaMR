...
def check_data_sources(args, cfg):
    ...
    a = time.time()
    if args.type == "video":
        preprocess_frames(args.sources.images, args.src_path)
    b = time.time()
    preprocess_tracks(
        cfg.datatype,
        args.sources.images,
...
class MultiPeopleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(
            len(self.track_ids),
            self.track_ids[idx],
            obs_data["is_right"].shape,
            obs_data["seq_interval"].shape,
        )
...
class CameraData(object):
    def __init__(
        ...
        is_static: bool,
        data_interval: Tuple[int, int] = (0, -1),
        track_interval: Tuple[int, int] = (0, -1),
    ):
